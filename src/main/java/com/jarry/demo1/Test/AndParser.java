...
public class AndParser implements Parser {
    ...
    public int doParse(String sqlString,int canMatch) {
        ...
        for (int i = 0; i < split.length; i+=3) {
            ...
            switch (op){
                case ">": if (Integer.valueOf(value) < Integer.valueOf(map.get(key))) tmpCanMatch = 1;break;
                case "<": if (Integer.valueOf(value) > Integer.valueOf(map.get(key))) tmpCanMatch = 1;break;
                case "=": if (value.equals(map.get(key))) tmpCanMatch = 1;break;
                case "like": if (map.get(key).contains(value.replaceAll("%","")))tmpCanMatch = 1;break;
            }
